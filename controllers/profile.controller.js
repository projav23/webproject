...
const getProfile = async (req, res, next) => {
  ...
  const chartGames = await Matches.find({$and: [{acceptedGuests:{$in: userId}},{status: "Finalizado"}]})
  ...
  const matchStats = await Matches.find({$and: [{acceptedGuests:{$in: userId}},{status: "Finalizado"}]}).sort({date:-1}).limit(10)
